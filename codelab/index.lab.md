...
Open `.env` and set your API key and enable debug logging:
...
`DEBUG=true` logs every streaming event and HTTP call to the backend terminal. It is optional but recommended for this codelab: the verify steps reference log output that requires it.
...
```text
Error: TODO 1 not implemented: create genai.Client() in _make_client()
```

That error is expected. It confirms the app is correctly wired to your backend and ready for you to implement the TODO sections.
...
> aside positive
>
> **Indentation matters.** Every code block in this codelab is shown with the exact indentation it needs in the file. Match the leading whitespace shown, don't paste at column 0.
...
Replace the empty `kwargs: dict = {}` (8-space indentation) with:
...
Click **Run Digest** in the browser. You should no longer see the instant `TODO 2 and 3 not implemented yet` error. The real API call is now starting: you may see `⚡ Interaction started` or an API error about a missing environment parameter. Either way, the guard is bypassed and the live call is wired.

---

## Step 3: Add Environment and Voice
...
Add the following directly below the `kwargs: dict = {...}` block from Step 2, at the same 8-space indentation:
...
## Step 4: Extract IDs from the Completed Event
...
Replace `pass` (8-space indentation) with:
...
> aside negative
>
> **Don't add a `return` here.** The function already ends with `return env_id, iid` after this entire `if`/`elif` chain, at the same 4-space indentation as `elif`. That line returns `(None, None)` for every event type except `interaction.completed`, and your extracted IDs for `interaction.completed`. If you add another `return` inside this block, especially nested inside `if usage:`, `_handle_event` returns `None` for all other event types, and the caller's `e, i = _handle_event(...)` crashes with "cannot unpack non-iterable NoneType object" on the very first `interaction.created` event.
...
## Step 6: Save and List Managed Agents
...
Replace it (4-space indentation) with:
...
The same Managed Agents API is also available on the **Agent Platform**, Google's enterprise surface for
...
> aside positive
>
> **Naming note.** The product is called the **Agent Platform** (or "Gemini Enterprise Agent Platform"). The Python
> SDK package is still `google-genai`: you initialise the client with `genai.Client(enterprise=True, project=...,
> location=...)` instead of an API key, and requests are routed to the `aiplatform.googleapis.com` endpoint under
> your GCP project.
...
Expected log line:
```text
INFO  digest  Surface: Agent Platform (project=your-project-id)
...
| Extra parameters | None | `background=True`, `store=True` |
...
- [Agent Platform documentation](https://cloud.google.com/gemini-enterprise-agent-platform/build/managed-agents)
